...
// Primary alias / display title for an artifact — the single naming source that
// both generateFrontmatter and the title upsert mirror.
export function artifactTitle(artifact) {
  const { kind, changeId: id, capability: cap } = artifact;
  if (kind === "spec") return `${cap} spec`;
  if (kind === "proposal") return `${id} proposal`;
  if (kind === "design" || kind === "tasks") return `${id} ${kind}`;
  if (kind === "delta") return `${id} ${cap} delta`;
...
// Insert a `title:` line into an existing frontmatter fence: after the `type:`
// line when present, else immediately after the opening fence. Line-based,
// never YAML-parsed, so the diff is exactly one line.
function insertTitle(text, title) {
  const lines = text.split("\n");
...
export function backfill(root, { dryRun = false } = {}) {
  ...
  for (const file of files) {
    ...
    if (text.split("\n")[0].trim() === "---") {
      ...
      if (region === null || /^title:/m.test(region)) {
        console.log(`SKIP ${rel} (has frontmatter)`);
        skipped.push(rel);
        continue;
      }
      // Frontmatter present but no title: upsert exactly the missing key.
      if (dryRun) {
        console.log(`ADD ${rel} (title, dry-run, not written)`);
      } else {
        fs.writeFileSync(file, insertTitle(text, artifactTitle(artifact)));
        console.log(`ADD ${rel} (title)`);
      }
      written.push(rel);
      continue;
    }
