...
export function init(root, { force = false } = {}) {
  ...
  // Feature toggle file: seed with all optional features off when absent, but
  // NEVER overwrite (even with --force) — it records the adopter's choices, not
  // reinstallable scaffolding.
  const togglePath = path.join(root, "openspec", "obsidian.yaml");
  if (fs.existsSync(togglePath)) {
    console.log("SKIP openspec/obsidian.yaml (exists; feature choices are never overwritten)");
  } else {
    fs.copyFileSync(path.join(ASSETS, "obsidian.yaml"), togglePath);
    console.log("WROTE openspec/obsidian.yaml");
  }
