...
Single entrypoint dispatches to three self-contained lib modules:
...
- Each lib module **re-derives `path.join(root, "openspec")` locally** — there is deliberately no shared `paths` helper. `walkMd(dir)` (recursive `.md` collector) and the wikilink regex `/\[\[([^\]|]+)(?:\|[^\]]*)?\]\]/g` are duplicated verbatim across `backfill.mjs` and `archive.mjs`.
...
| `lib/` | Command implementations: `init.mjs`, `backfill.mjs`, `archive.mjs` |
| `assets/` | Static data installed by `init`: `schema.yaml`, `config-rules.yaml`, `templates/*.md` |
| `assets/templates/` | Obsidian-aware artifact scaffolds: `proposal.md`, `spec.md`, `design.md`, `tasks.md` |
| `test/` | `node:test` suites, one per lib module (`archive.test.mjs`, `backfill.test.mjs`) |
...
Flags: `--root <dir>` (all commands, default cwd), `--dry-run` (`backfill` only), `--force` (`init` only, overwrite existing schema/template files).
...
5. **Sync + Archive (automatic, at apply-time)** — the final `/opsx:apply` step (all tasks
   ...
   and rewrites its wikilinks; then `npm run opsx:validate` must pass and the results
...
6. **PR** — push, open a PR titled with Conventional Commits and fill in the PR template
   ...
   archive gate (`npm run opsx:check`); on a push to `main`, GitVersion tags the release commit.
...
- **Typed error classes as the failure channel.** Each lib exports `class XError extends Error {}` (`InitError`, `BackfillError`, `ArchiveError`) and *throws* on failure. `bin/cli.mjs` catches: known error → print `err.message`; unknown → print raw `err`; then `process.exit(1)`.
- **Exit codes:** `0` ok, `1` failure (any thrown error), `2` usage (bad/missing command → prints `USAGE` to stderr).
- **Status logging via verb-prefixed lines** to stdout: `WROTE`, `SKIP … (exists; use --force)`, `ADD`, `ARCHIVED`, `NOTHING TO ARCHIVE`, `CHECK OK`, `BACKFILL OK`. Errors go to stderr.
...
- **Structure:** one test file per lib module (`archive.test.mjs`, `backfill.test.mjs`). Top-level `test(name, async (t) => …)`; `archive.test.mjs` nests `await t.test("sub-case", …)` for related sub-scenarios (numbered cases 1–8).
