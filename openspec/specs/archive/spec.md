...
The `archive` command completes a change: it merges the change's delta specs into the live specs under `openspec/specs/` and moves the change into a dated `changes/archive/` directory, rewriting its intra-change wikilinks so navigation survives the move. It is deterministic and needs no OpenSpec CLI or git.
